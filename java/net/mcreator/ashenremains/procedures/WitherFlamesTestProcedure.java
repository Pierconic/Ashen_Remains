			...
			for (int index0 = 0; index0 < 30; index0++) {
				...
				if ((world.getBlockState(BlockPos.containing(x + sx, y - sy, z + sz))).is(BlockTags.create(ResourceLocation.parse("forge:ghast_replaceable")))) {
					if (Math.random() < 0.05) {
						...
						world.setBlock(BlockPos.containing(x + sx, y - sy, z + sz), AshenremainsModBlocks.ACTIVATED_SOUL_SOIL.get().defaultBlockState(), 3);
					} else if (Math.random() < 0.6) {
						world.setBlock(BlockPos.containing(x + sx, y - sy, z + sz), Blocks.SOUL_SAND.defaultBlockState(), 3);
						if (Math.random() < 0.1 && world.isEmptyBlock(BlockPos.containing(x + sx, y - sy + 1, z + sz))) {
							world.setBlock(BlockPos.containing(x + sx, y - sy + 1, z + sz), Blocks.SOUL_FIRE.defaultBlockState(), 3);
						} else if (Math.random() < 0.1 && world.isEmptyBlock(BlockPos.containing(x + sx, y - sy + 1, z + sz))) {
							world.setBlock(BlockPos.containing(x + sx, y - sy + 1, z + sz), AshenremainsModBlocks.WEIRD_FIRE.get().defaultBlockState(), 3);
						}
					} else {
